 ...
 * Run with: node time-controller-clean-demo.js
 */
...
console.log('  Start:', timeRange.startTime?.toLocaleTimeString());
console.log('  End:', timeRange.endTime?.toLocaleTimeString());
console.log('');
...
locales.forEach(locale => {
  ...
  console.log(`✅ ${locale.name} locale (${locale.code}):`, controller.methods.formatTime(testTime));
});
console.log('');
...
const timeSelectedUnsubscribe = eventController.events.timeSelected.subscribe((selectedTime) => {
  console.log('🔔 Time selected event:', selectedTime.toLocaleTimeString());
});
...
eventTime.setHours(16, 45, 30);
eventController.selectTime(eventTime);
...
console.log('✅ Time view has hours, minutes, seconds segments for UI');
console.log('');

// ========================================
// 12. Real-world Usage Examples
// ========================================
console.log('12. 🌟 Real-world Usage Examples');
console.log('--------------------------------');

// Business meeting scheduler
const meetingScheduler = TimeController({
  use12HourFormat: true,
  showSeconds: false,
  minuteStep: 30,
  isRangeSelection: true
...
businessStart.setHours(8, 0, 0);
const businessEnd = new Date();
businessEnd.setHours(18, 0, 0);

meetingScheduler.methods.setMinTime(businessStart);
meetingScheduler.methods.setMaxTime(businessEnd);
meetingScheduler.methods.setDisabledHours([12]); // Lunch break

console.log('✅ Meeting Scheduler Configuration:');
console.log('  - Business hours: 8 AM - 6 PM');
console.log('  - 30-minute slots');
console.log('  - Lunch break blocked (12 PM)');

// Schedule a meeting
const meetingStart = new Date();
meetingStart.setHours(10, 0, 0);
meetingScheduler.selectTime(meetingStart);

const meetingEnd = new Date();
meetingEnd.setHours(11, 30, 0);
meetingScheduler.selectTime(meetingEnd);

console.log('✅ Meeting scheduled:');
console.log('  Start:', meetingScheduler.selectedTimeRange.startTime?.toLocaleTimeString());
console.log('  End:', meetingScheduler.selectedTimeRange.endTime?.toLocaleTimeString());
console.log('');
